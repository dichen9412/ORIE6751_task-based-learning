...
def linear_softmax_reg(X, Y, params):
    ...
    f = cp.sum(cp.log_sum_exp(X@Theta, axis=1) -
                       cp.sum(cp.multiply(Y, X@Theta), axis=1)) / m #this is negative log-likelihood
...
def newsvendor_eval(X, Y, Theta_lin, Theta_sq, params):
    ...
    for i in range(m):
        z_buy[i], _ = newsvendor_opt(params, py[i]) #get z from y_pred
        f_eval[i] = f_obj(z_buy[i], params['d'].dot(Y[i]), params) #loss(z, y_true)
        z_buy_opt, f_opt[i] = newsvendor_opt(params, Y[i]) #get z_opt for y_true

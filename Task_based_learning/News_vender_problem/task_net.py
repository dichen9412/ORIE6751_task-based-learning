...
def run_task_net(X, Y, X_test, Y_test, params, is_nonlinear=False, lam=10.0):
    ...
    for i in range(1000):
        model.train()
        train_cost, train_nll = batch_train(100, i, X_train_t, Y_train_t,
        ...
        if 100 * i > 400 and abs(i - last_best) > 5:
            idx = hold_costs.index(min(hold_costs))
